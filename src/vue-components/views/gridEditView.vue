<template>
    <div class="box">
        ...
        <div style="display: none">
            <button id="moreButton" :aria-label="$t('more')" class="spaced">
        ...
        <header class="d-flex align-items-center transfer-props-header" role="toolbar" v-if="propTransferObject">
            <div class="me-5">
                <strong class="me-2">
                    <i class="fas fa-angle-double-right"></i>&nbsp;<span class="d-none d-sm-inline-block"
                        ><span>{{ $t('transferProperties') }}</span
                        >:</span
                    >
                </strong>
                <span
                    ><span>{{ $t('selectTargetElements') }}</span
                    >...</span
                >
            </div>
            <button class="mb-0 me-2" @click="stopPropTransfer" :title="$t('cancel')">
                <i class="fas fa-times" /> <span class="d-none d-lg-inline-block">{{ $t('cancel') }}</span>
            </button>
            <button class="mb-0" @click="applyPropTransfer" :title="$t('ok')">
                <i class="fas fa-check" /> <span class="d-none d-lg-inline-block">{{ $t('ok') }}</span>
            </button>
        </header>

        <!-- Element Action Toolbar -->
        <element-action-toolbar
            v-if="!propTransferObject && (markedElementIds.length > 0 || (selectionMode && selectedCells.length > 0))"
        ...
        <div>
            <grid-settings-modal
                v-if="showGridSettingsModal"
                :grid-data-param="gridData"
                :is-global-grid="metadata.globalGridId === gridData.id"
                @reload="reload"
                @close="showGridSettingsModal = false"
                :undo-service="undoService"
            />
        </div>
        <div>
...
let vueConfig = {
    ...
    methods: {
        ...
        moveAll: function (dir) {
            ...
            this.gridData.gridElements = gridLayoutUtil.moveAsPossible(
        ...
        async duplicateElements(shouldIncludeId = null) {
            ...
            if (duplicates.length === 1) {
                this.gridData.gridElements = gridLayoutUtil.insertDuplicate(
                    this.gridData.gridElements,
                    elements[0],
                    duplicates[0],
                    {
        ...
        async newElement(type, useInteractionPos) {
            if (type === GridElement.ELEMENT_TYPE_NORMAL) {
                this.editElementId = null;
                this.newPosition =
                    useInteractionPos && this.lastInteraction.x !== undefined ? this.lastInteraction : null;
                this.showEditModal = true;
            } else {
                let showEdit = false;
        ...
        async pasteElements(elements = null) {
            ...
            if (!elements.length) {
                ...
                element.height = 1;
                element.x =
                    this.lastInteraction.x !== undefined
                        ? this.lastInteraction.x
                        : gridUtil.getHeightWithBounds(this.gridData);
        ...
        onKeyDown(event) {
            if (
                ...
                this.showGridSettingsModal ||
                this.showNavigateModal ||
                this.showTranslateModal ||
                this.showPropTransferModal ||
                this.showEditModal
            ) {
                return;
            }
...
function initContextmenu() {
    ...
    let itemsTransferProps = {
        ...
        CONTEXT_PROPERTY_TRANSFER_APPEARANCE: {
            name: i18nService.t('transferAppearance'),
    ...
    let itemsElemNormal = {
        ...
        TRANSFER_PROPS: {
            name: i18nService.t('transferProperties'),
            icon: 'fas fa-angle-double-right',
            items: itemsTransferProps,
            visible: visibleTransferPropsFn
        },
        SEP1: '---------',
        ...
        CONTEXT_GRID_NAVIGATION: {
            ...
            visible: visibleNormalFn
        },
        CONTEXT_ACTION_DO_ACTION: {
            name: i18nService.t('doElementAction'),
            icon: 'fas fa-bolt',
            visible: visibleNormalFn
        }
    };
    ...
    var itemsMoreMenuButton = {
        SELECTED_ELEM_ACTIONS: {
            name: i18nService.t('selectedElementsContextMenu'),
            icon: 'far fa-square',
            visible: visibleFn,
